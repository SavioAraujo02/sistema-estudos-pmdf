...
export default function LoginPage() {
  ...
  const PIX_CHAVE = "a53256a0-24a6-4c68-9322-61dc1e0e04e8"
...
// Adicionar este useEffect para forçar re-render quando tempo acabar
useEffect(() => {
...
useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Verificar tentativas apenas se não há tempo restante
    if (tentativasLogin >= MAX_TENTATIVAS && tempoRestante === 0) {
      // Limpar tentativas se chegou aqui (failsafe)
      localStorage.removeItem('login_tentativas')
      localStorage.removeItem('bloqueio_ate')
      setTentativasLogin(0)
    }
    ...
    try {
      // 1. Verificar se o usuário existe na nossa tabela
      ...
      if (checkError || !usuarioExiste) {
        setTentativasLogin(prev => {
          ...
          if (novas >= MAX_TENTATIVAS) {
            const agora = new Date()
            const bloqueioAte = new Date(agora.getTime() + 15 * 60 * 1000) // 15 minutos
            localStorage.setItem('bloqueio_ate', bloqueioAte.toISOString())
            setBloqueadoAte(bloqueioAte)
            setTempoRestante(15 * 60) // 15 minutos em segundos
          }
      ...
      if (error) {
        ...
        await supabase.from('historico_logins').insert({
          ...
          ip_address: 'Unknown', // Será preenchido pelo sistema de sessões
          sucesso: true,
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
      ...
      {/* Indicador de conectividade - só renderiza no cliente */}
      {isClient && (
          ...
          {modo === 'login' && (
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-3 py-2 pr-10 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="••••••••"
                    required
              ...
              <button
                ...
                className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {loading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
            ...
            <form onSubmit={handleCadastro} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="Ex: Pelotão 11, Pelotão 12"
                />
              </div>

              <div>
                <label htmlFor="email-cadastro" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  <Mail className="h-4 w-4 inline mr-1" />
                  Email *
                </label>
                <input
              ...
              <div>
                <label htmlFor="senha-cadastro" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  <Lock className="h-4 w-4 inline mr-1" />
                  Senha *
                </label>
                <div className="relative">
              ...
              <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                ...
                <ul className="text-xs text-blue-800 dark:text-blue-400 space-y-1">
                  <li>• Valor: {VALOR_CURSO} (pagamento único)</li>
                  <li>• Pagamento via PIX</li>
                  <li>• Acesso liberado após confirmação</li>
              ...
              <button
                type="submit"
                disabled={loading || !validarSenhaForte()}
                className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {loading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
          ...
          {modo === 'reset-senha' && (
            <div className="space-y-4">
              ...
              <form onSubmit={handleResetSenha} className="space-y-4">
                <div>
                  <label htmlFor="email-reset" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Email
                  </label>
                  <input
                    id="email-reset"
          ...
          {modo === 'trocar-senha' && (
            <div className="space-y-4">
              ...
              <form onSubmit={handleTrocarSenha} className="space-y-4">
                <div>
                  <label htmlFor="nova-senha" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Nova Senha
                  </label>
                  <div className="relative">
                    <input
                ...
                <div>
                  <label htmlFor="confirmar-senha" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
